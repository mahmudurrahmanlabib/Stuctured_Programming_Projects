...
struct luna_airline
{
	...
	int seat_num;
	char email[15];
	struct luna_airline *following;
}
*begin, *stream;
struct luna_airline *dummy;
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct luna_airline*)malloc(sizeof(struct luna_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct luna_airline *)malloc(sizeof(struct luna_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
